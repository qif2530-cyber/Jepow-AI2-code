...
const requiredFiles = [
  ...
  'package.json',
  'src/lib/runtime.ts',
  'src/components/ThreeDEditorNode.tsx',
  'src/components/JepowViewportPreview.tsx',
];
...
const editorNode = read('src/components/ThreeDEditorNode.tsx');
const viewportPreview = read('src/components/JepowViewportPreview.tsx');
for (const expected of [
  ...
  assert(
    editorNode.includes(expected) || viewportPreview.includes(expected),
    `Canvas 3D node preview missing architecture integration: ${expected}`,
  );
}
...
for (const expected of [
  ...
  assert(app.includes(expected), `App missing infinite-canvas native 3D pipeline integration: ${expected}`);
}
