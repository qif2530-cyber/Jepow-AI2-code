...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
  ...
  let ndoth = max(dot(n, half_dir), 0.001);
  let vdoth = max(dot(view_dir, half_dir), 0.001);
  let alpha = max(roughness * roughness, 0.002);
  let alpha2 = alpha * alpha;
  let denom = max((ndoth * ndoth) * (alpha2 - 1.0) + 1.0, 0.001);
  let d_ggx = alpha2 / (3.14159265 * denom * denom);
  let k = ((roughness + 1.0) * (roughness + 1.0)) / 8.0;
  let g_l = ndotl / max(ndotl * (1.0 - k) + k, 0.001);
  let g_v = ndotv / max(ndotv * (1.0 - k) + k, 0.001);
  let f0 = mix(vec3<f32>(0.04 + specular * 0.08), base, metalness);
  let fresnel = f0 + (vec3<f32>(1.0) - f0) * pow(1.0 - vdoth, 5.0);
  let spec_brdf = (d_ggx * g_l * g_v) * fresnel / max(4.0 * ndotl * ndotv, 0.001);
  let diffuse_color = mix(base, vec3<f32>(0.78, 0.86, 0.95), transmission * 0.38);
  let diffuse_brdf = diffuse_color * (1.0 - metalness) / 3.14159265;
  let env_fill = vec3<f32>(0.55, 0.62, 0.70) * environment * (0.16 + roughness * 0.22);
  let coat = pow(ndoth, mix(90.0, 420.0, 1.0 - roughness)) * clearcoat * (0.25 + 0.75 * ndotl);
  let rim = vec3<f32>(0.08, 0.10, 0.12) * pow(1.0 - ndotv, 5.0) * (1.0 + clearcoat);
  let direct = (diffuse_brdf + spec_brdf) * diffuse * ndotl * 3.14159265;
  let ambient = diffuse_color * uniforms.ambient * 0.35;
  let lit = direct + ambient + env_fill + rim + vec3<f32>(coat) + base * emission;
  let mapped = vec3<f32>(1.0) - exp(-max(lit * exposure, vec3<f32>(0.0)));
